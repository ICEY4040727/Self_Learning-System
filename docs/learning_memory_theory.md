...
**理论基础**：根据 MSKT 理论，元认知技能是跨领域稳定的特质。

```python
def compute_metacognition_trend(world_profiles: list[dict]) -> dict:
    """
    计算元认知趋势 - 展示成长而非快照
    ...
    for dim in MSKT_DIMENSIONS:
        ...
        if len(dim_records) >= 2:
            dim_records.sort(key=lambda x: x["t_updated"])
            earliest = normalize_value(dim_records[0]["value"])
            latest = normalize_value(dim_records[-1]["value"])
            trend = "improving" if latest > earliest else "stable"
            
            result[dim] = {
                "current": dim_records[-1]["value"],
                "trend": trend,
                "evidence_count": len(dim_records)
            }
        else:
            result[dim] = {
                "current": dim_records[0]["value"],
                "trend": "unknown",
                "evidence_count": 1
            }
...
#### 偏好稳定性（ITS Learner Characteristics）

**理论基础**：根据 ITS 研究，学习偏好是跨情境稳定的特征。

```python
def compute_preference_stability(world_profiles: list[dict]) -> dict:
    """
    ...
    for trait in PREFERENCE_TRAITS:
        ...
        for profile in world_profiles:
            ...
            if trait_data:
                trait_values.append(trait_data.get("value"))
        ...
        # 布尔值：计算一致率
        if all(isinstance(v, bool) for v in trait_values):
            true_count = sum(1 for v in trait_values if v)
            consistency = true_count / len(trait_values)
            result[trait] = {
        ...
        # 枚举值：找出最常见的
        elif all(isinstance(v, str) for v in trait_values):
            from collections import Counter
            most_common = Counter(trait_values).most_common(1)[0]
            result[trait] = {
                "stable": most_common[1] / len(trait_values) >= 0.7,
...
def compute_learning_stats(
    ...
    for profile in world_profiles:
        ...
        # 从知识图谱获取概念数
        knowledge = knowledge_service.get_knowledge(db, world_id)
        ...
        # 计算该世界的平均掌握度
        if concepts:
            mastery_sum = sum(c.get("mastery", 0) for c in concepts.values() if isinstance(c, dict))
...
{
  "raw_worlds": {
    "1": { "metacognition": {...}, "session_count": 5 },
    "2": { "metacognition": {...}, "session_count": 3 }
  },
  "aggregated": {
    "metacognition_trend": {...},
...
| 操作 | 行为 |
|------|------|
| 每条消息后 | 更新该世界的 raw_worlds → 重新聚合 |
| 会话结束时 | 增量+1 session_count → 更新 learning_stats.total_sessions |
| 用户查看时 | 懒计算，超过24小时重算 |
...
|---------|------|
| 每个会话结束后 | 更新会话统计 |
| LearnerProfile 更新时 | 触发元认知/偏好重新计算 |
| 用户查看成长报告时 | 懒计算（按需） |
| 跨世界成就触发 | 周期性检查 |

### 5.5 呈现方式（详见 UI 设计文档）

待讨论。参见 `design_UI.md` 中的"成长报告"部分。

...
class TestCourseSagesAPI:
    ...
    def test_get_course_sages_returns_world_level_sages(self, client, auth_headers):
        """验证返回世界级别的 Sage 角色"""
        # 创建测试数据
        world_resp = client.post("/api/worlds", json={"name": "TestWorld"}, headers=auth_headers)
        world_id = world_resp.json()["id"]

        # 创建 Sage 角色并绑定到世界
        sage_resp = client.post("/api/character", json={
            "name": "TestSage",
            "type": "sage",
            "title": "导师"
        ...
        # 绑定到世界
        client.post(f"/api/worlds/{world_id}/characters", json={
        ...
        # 创建课程
        course_resp = client.post(f"/api/worlds/{world_id}/courses", json={
            "name": "TestCourse"
        }, headers=auth_headers)
        course_id = course_resp.json()["id"]

        # 调用 API
        resp = client.get(f"/api/courses/{course_id}/sages", headers=auth_headers)
    ...
    def test_get_course_sages_ignores_unbound_meta_sage_ids(self, client, auth_headers):
        ...
        # 创建多个 Sage
        sage1_resp = client.post("/api/character", json={
            "name": "Sage1", "type": "sage"
        }, headers=auth_headers)
        sage1_id = sage1_resp.json()["id"]

        sage2_resp = client.post("/api/character", json={
...
class TestCourseSessionsAPI:
    ...
    def test_get_course_sessions_empty(self, client, auth_headers):
        """验证空会话列表返回"""
        world_resp = client.post("/api/worlds", json={"name": "TestWorld3"}, headers=auth_headers)
        world_id = world_resp.json()["id"]
        ...
        resp = client.get(f"/api/courses/{course_id}/sessions", headers=auth_headers)
    ...
    def test_get_course_sessions_returns_sessions(self, client, auth_headers):
        """验证返回课程关联的会话"""
        # 创建基础数据
        world_resp = client.post("/api/worlds", json={"name": "TestWorld4"}, headers=auth_headers)
        world_id = world_resp.json()["id"]

        # 创建并绑定 Sage 和 Traveler（start_learning 需要）
        sage_resp = client.post("/api/character", json={
            "name": "SessionSage", "type": "sage"
        }, headers=auth_headers)
        client.post(f"/api/worlds/{world_id}/characters", json={
            "character_id": sage_resp.json()["id"],
            "role": "sage", "is_primary": True
        }, headers=auth_headers)

        traveler_resp = client.post("/api/character", json={
            "name": "SessionTraveler", "type": "traveler"
        }, headers=auth_headers)
        client.post(f"/api/worlds/{world_id}/characters", json={
            "character_id": traveler_resp.json()["id"],
            "role": "traveler", "is_primary": True
        }, headers=auth_headers)
...
class TestCourseMemoryFactsAPI:
    ...
    def test_get_memory_facts_stats_only(self, client, auth_headers, db_session: Session):
        ...
        # 创建测试数据
        world_resp = client.post("/api/worlds", json={"name": "TestWorld5"}, headers=auth_headers)
        world_id = world_resp.json()["id"]
        ...
        # 调用 API
        resp = client.get(f"/api/courses/{course_id}/memory-facts", params={"stats_only": "true"}, headers=auth_headers)
    ...
    def test_get_memory_facts_with_list(self, client, auth_headers, db_session: Session):
        ...
        world_resp = client.post("/api/worlds", json={"name": "TestWorld6"}, headers=auth_headers)
        world_id = world_resp.json()["id"]
        ...
        # 不带 stats_only
        resp = client.get(f"/api/courses/{course_id}/memory-facts", headers=auth_headers)

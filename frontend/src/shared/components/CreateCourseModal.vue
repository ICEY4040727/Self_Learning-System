          ...
          <div class="field-group">
            ...
            <div v-if="bookshelfItems.length > 0" class="textbook-list">
              <div
                ...
                :key="item.id"
                class="textbook-item"
                :class="{ selected: selectedBookIds.includes(item.id), disabled: !item.is_usable, error: item.status === 'error' }"
                @click="toggleBook(item)"
              >
                <div class="textbook-main">
                  <span class="textbook-name">{{ item.title || item.filename }}</span>
                  <span v-if="item.error_message" class="textbook-error">{{ item.error_message }}</span>
                </div>
                <span v-if="item.is_usable" class="textbook-status ok">已解析</span>
                <span v-else class="textbook-status error">不可用</span>
            ...
            <div class="upload-area" @click="triggerUpload" @dragover.prevent @drop.prevent="handleDrop">
              <input ref="fileInput" type="file" hidden :accept="TEXTBOOK_ACCEPT" @change="handleFileSelect" />
              <div v-if="uploading" class="upload-progress">
                <div class="progress-bar"><div class="progress-fill" :style="{ width: uploadProgress + '%' }"></div></div>
                <span>上传 / 解析中 {{ uploadProgress }}%</span>
              </div>
              <div v-else class="upload-hint">+ 点击或拖拽上传教材 ({{ TEXTBOOK_UPLOAD_HINT }})</div>
            </div>
            <div v-if="uploadNotice" class="upload-message" :class="uploadNoticeType">{{ uploadNotice }}</div>
          </div>
...
const uploadNoticeType = ref<'success' | 'error'>('success')
...
function toggleBook(item: BookshelfItem) {
  if (!item.is_usable) {
    uploadNoticeType.value = 'error'
    uploadNotice.value = item.error_message
      ? `教材解析失败：${item.error_message}`
      : '该教材不可用于课程生成'
    return
  }
  const id = item.id
...
async function uploadFile(file: File) {
  ...
  try {
    ...
    if (item.is_usable) {
      ...
      uploadNoticeType.value = 'success'
      uploadNotice.value = `教材「${item.title || item.filename}」已加入书架`
    } else {
      uploadNoticeType.value = 'error'
      uploadNotice.value = item.error_message
        ? `教材已上传，但解析失败：${item.error_message}`
        : '教材已上传，但当前不可用于课程生成'
    }
  } catch (err: any) {
    console.error('Upload failed:', err)
    uploadNoticeType.value = 'error'
    uploadNotice.value = err?.response?.data?.detail || err?.message || '上传失败'
  } finally {
...
async function handleCreateAndGenerate() {
  ...
  try {
    ...
    const usableSelectedBookIds = selectedBookIds.value.filter((id) =>
      bookshelfItems.value.some((item) => item.id === id && item.is_usable),
    )
    if (selectedBookIds.value.length > 0 && usableSelectedBookIds.length === 0) {
      genError.value = '所选教材均不可用，请重新选择'
      aiStage.value = '教材关联失败'
      genStatus.value = 3
      return
    }
    if (usableSelectedBookIds.length > 0) {
      try {
        await bookshelfApi.batchLinkToCourse(course.id, usableSelectedBookIds)
      } catch (err: any) {
        console.error('Failed to link textbooks:', err)
        genError.value = err?.response?.data?.detail || err?.message || '教材关联失败'
        aiStage.value = '教材关联失败'
        genStatus.value = 3
        return
      }

    ...
    <Transition name="dialog-slide">
      ...
      >
        <div style="padding:0 16px;">
          <DialogBox
            :mode="dialogMode"
            :character-name="store.sageName"
            :display-content="store.currentText"
            :choices="store.currentChoices"
            :has-more-segments="false"
            :is-typing="store.thinking && store.mode === 'speaking'"
            @click-next="handleContinue"
            @select-choice="handleChoiceSelect"
            @send-message="store.sendMessage"
            @skip-typing="skipSignal++"
          />
        </div>
      </div>
    </Transition>
...
import { useLearningStore } from '@/app/stores/learning'
import { useSettingsStore } from '@/settings/stores/settings'
import CharacterSprite from '@/characters/components/CharacterSprite.vue'
import DialogBox from '@/shared/components/galgame/DialogBox.vue'
import HudBar from '@/shared/components/HudBar.vue'
import BacklogPanel from '@/shared/components/galgame/BacklogPanel.vue'
import SaveLoadPanel from '@/shared/components/SaveLoadPanel.vue'
import KnowledgeGraphModal from '@/shared/components/KnowledgeGraphModal.vue'
import RelationshipStageOverlay from '@/shared/components/RelationshipStageOverlay.vue'
import MemoryFactsDrawer from '@/courses/components/course/MemoryFactsDrawer.vue'
...
const dialogMode = computed(() => {
  if (store.mode === 'speaking') return 'TEACHER_SPEAKING'
  if (store.mode === 'input') return 'USER_INPUT'
  if (store.mode === 'choices') return 'CHOICES'
  return 'WAITING'
})

...
watch(() => props.show, (newVal) => {
  if (newVal && props.character) {
    ...
    form.llm.provider = props.character.llm_settings?.provider || 'claude'
    form.llm.model = props.character.llm_settings?.model || ''
    form.llm.base_url = props.character.llm_settings?.base_url || ''
    form.llm.temperature = props.character.llm_settings?.temperature ?? 0.7
    form.llm.max_tokens = props.character.llm_settings?.max_tokens ?? 2048
  }
})
...
const handleSubmit = async () => {
  ...
  const data: UpdateData = {
    ...
    llm_settings: props.character.type === 'sage'
      ? {

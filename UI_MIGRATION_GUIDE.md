...
> 目的：为当前仓库 UI 迁移提供“业务不缩水”的统一基线（后端契约 + UI_figma 视觉交互规范）。  
> 约束：迁移 UI 时，后端业务能力与关键响应字段不得减少；只能增量扩展，不能破坏现有契约。

## 1. 全局约定（必须保持）

- **API 前缀**：`/api`（认证为 `/api/auth/*`）
- **鉴权**：除登录/注册外，前端请求均带 `Authorization: Bearer <token>`
- **时间字段**：ISO 8601 字符串（前端直接 `new Date(...)`）
- **错误格式**：FastAPI 默认 `{"detail": "..."}`，前端统一按该结构解析
- **核心路由挂载**：
...
### 2.1 认证（Login/Auth Store）

| 接口 | 方法 | 请求 | 关键响应字段 |
|---|---|---|---|
| `/api/auth/register` | POST | `username`, `password` | `id`, `username`, `role` |
| `/api/auth/login` | POST | 表单字段 `username`, `password` | `access_token`, `token_type` |
| `/api/auth/me` | GET | Bearer Token | `id`, `username`, `role` |

### 2.2 Home 世界主链（world-first）

| 接口 | 方法 | 用途 | 关键字段 |
|---|---|---|---|
| `/api/worlds` | GET | 世界列表 | `id`, `name`, `description`, `scenes` |
| `/api/worlds/{world_id}/courses` | GET | 世界内课程列表 | `id`, `world_id`, `name`, `description`, `target_level` |
| `/api/worlds/{world_id}/timelines` | GET | 时间线树 | `sessions[]`, `checkpoints[]`（见下） |
| `/api/checkpoints/{checkpoint_id}/branch` | POST | 从存档分叉开新会话 | `session_id`, `course_id`, `world_id`, `parent_checkpoint_id`, `branch_name` |

`/timelines` 当前前端依赖的子字段：
- `sessions[*]`: `id`, `course_id`, `parent_checkpoint_id`, `branch_name`, `started_at`, `relationship_stage`
- `checkpoints[*]`: `id`, `session_id`, `save_name`, `message_index`, `created_at`

### 2.3 Learning 学习主链（对话/情感/关系/图谱）

| 接口 | 方法 | 用途 | 关键字段 |
|---|---|---|---|
| `/api/courses/{course_id}/start` | POST | 启动/复用学习会话 | `session_id`, `teacher_persona`, `course`, `relationship_stage`, `relationship`, `greeting`, `scenes`, `sage_sprites`, `traveler_sprites` |
| `/api/sessions/{session_id}/history` | GET | 历史消息 | `id`, `sender_type`, `content`, `timestamp` |
| `/api/courses/{course_id}/chat` | POST | 发送消息 | `type`, `reply`, `choices?`, `emotion?`, `relationship_stage?`, `relationship?`, `relationship_events?`, `expression_hint?` |
| `/api/courses/{course_id}` | GET | 回填 world_id | `world_id` |
| `/api/worlds/{world_id}/checkpoints` | GET | 学习页 checkpoint 列表 | `id`, `world_id`, `session_id`, `save_name`, `message_index`, `created_at` |
| `/api/checkpoints` | POST | 创建 checkpoint | 同上 |
| `/api/worlds/{world_id}/knowledge-graph` | GET | 知识图谱 | `nodes[]`, `edges[]` |

`/knowledge-graph` 当前图组件依赖：
- `nodes[*]`: `id`, `name`, `mastery`, `status`, `type`
- `edges[*]`: `source`, `target`, `type`
- 支持查询参数：`checkpoint_time`, `session_id`

### 2.4 Archive 档案页

| 接口 | 方法 | 用途 | 关键字段 |
|---|---|---|---|
| `/api/learning_diary` | GET/POST | 学习日记 | `id`, `course_id`, `date`, `content`, `reflection` |
| `/api/progress` | GET | 学习进度（支持 `course_id`） | `id`, `topic`, `mastery_level`, `next_review` |
| `/api/courses` | GET | 课程筛选数据源 | `id`, `name` |
| `/api/save` | GET | 存档列表（legacy） | `id`, `save_name`, `created_at` |
| `/api/save/{save_id}` | DELETE | 删除存档（legacy） | `message` |
| `/api/checkpoints/{checkpoint_id}/branch` | POST | 读档后分叉进入学习 | `course_id`, `world_id`, `session_id` |
| `/api/sessions` | GET | 情感轨迹会话列表 | `id`, `started_at`, `course_name` |
| `/api/sessions/{session_id}/emotion_trajectory` | GET | 情感轨迹数据 | `index`, `timestamp`, `emotion_type`, `valence`, `arousal`, `confidence` |

### 2.5 Character 角色/人格/课程管理

| 接口 | 方法 | 用途 | 关键字段 |
|---|---|---|---|
| `/api/character` | GET/POST | 角色列表/创建 | `id`, `name`, `type`, `personality`, `background`, `speech_style`, `sprites` |
| `/api/character/{character_id}` | PUT/DELETE | 编辑/删除角色 | 同上或 `message` |
| `/api/characters/{character_id}/sprites` | POST | 上传立绘（multipart） | 返回 `sprites` |
| `/api/teacher_persona` | GET/POST | 人格列表/创建（支持 `character_id` 查询） | `id`, `character_id`, `name`, `version`, `traits`, `system_prompt_template`, `is_active` |
| `/api/teacher_persona/{persona_id}` | PUT/DELETE | 编辑/删除人格 | 同上或 `message` |
| `/api/teacher_persona/{persona_id}/activate` | PUT | 激活人格 | `message` |
| `/api/persona/generate` | POST | AI 生成人格草稿 | `system_prompt_template`, `traits`, `name_suggestion` |
| `/api/worlds` | GET/POST | 世界列表/创建 | `id`, `name`, `description`, `scenes` |
| `/api/worlds/{world_id}/characters` | GET/POST | 世界角色绑定 | `id`, `world_id`, `character_id`, `role`, `is_primary`, `character_name` |
| `/api/worlds/{world_id}/courses` | GET/POST | 世界课程列表/创建 | `id`, `world_id`, `name`, `description`, `target_level` |
| `/api/courses/{course_id}` | PUT/DELETE | 编辑/删除课程 | 课程字段或 `message` |

`/characters/{id}/sprites` 上传约束（前端也要遵守）：
- 文件名 stem 仅允许：`default/happy/thinking/concerned`
- MIME：`image/png`, `image/jpeg`, `image/webp`
- 单文件大小：`<= 2MB`

`/persona/generate` 业务约束：
- `description` 长度 5~500
- 每用户 30 秒冷却（429）
- 必须先配置 `api_key`（否则 400）

### 2.6 Settings 设置页

| 接口 | 方法 | 请求 | 关键响应字段 |
|---|---|---|---|
| `/api/settings` | GET | Bearer Token | `default_provider` |
| `/api/settings` | PUT | `default_provider`, `api_key` | `message` |

---

## 3. 后端业务能力红线（即使 UI 改版也不能减）

1. **World-first 主流程**：世界 -> 课程 -> 学习会话 必须保留。
2. **Checkpoint 双模式**：`COMMIT`（创建）+ `BRANCH`（分叉）必须保留。
3. **分叉语义**：从 checkpoint 新建 session，继承课程/角色/关系，并按 `message_index` 复制历史消息。
4. **关系与情感联动**：`chat` 响应中的 `relationship*`、`emotion`、`expression_hint` 必须保留。
5. **双角色立绘链路**：学习开始返回 `sage_sprites`/`traveler_sprites`，支持前端同屏渲染。
6. **知识图谱链路**：世界维度图谱查询与会话/时间点过滤能力必须保留。
7. **用户隔离**：world/course/session/checkpoint 均按 `current_user.id` 严格隔离。
8. **世界知识初始化**：创建世界、世界绑定角色后，Knowledge 记录要可用（图谱接口不能空跑崩溃）。

---

## 4. Legacy 兼容约束（短期不可移除）

以下接口仍被当前前端使用，UI 迁移期间不能删除：
- `GET /api/save`
- `DELETE /api/save/{save_id}`

说明：新主链已是 `/checkpoints*`，但 Archive 页仍读取/删除 legacy save 视图。
...
如确需调整，必须先做“前端灰度兼容 + 双写/双读 + 迁移完成后下线”。
...
| 页面/组件 | 必需接口 |
|---|---|
| Login | `/api/auth/register`, `/api/auth/login`, `/api/auth/me` |
| Home | `/api/worlds`, `/api/worlds/{id}/courses`, `/api/worlds/{id}/timelines`, `/api/checkpoints/{id}/branch` |
| Learning | `/api/courses/{id}/start`, `/api/courses/{id}/chat`, `/api/sessions/{id}/history`, `/api/courses/{id}`, `/api/worlds/{id}/checkpoints`, `/api/checkpoints`, `/api/worlds/{id}/knowledge-graph` |
| Archive | `/api/learning_diary`, `/api/progress`, `/api/courses`, `/api/save`, `/api/save/{id}`, `/api/checkpoints/{id}/branch`, `/api/sessions`, `/api/sessions/{id}/emotion_trajectory` |
| Character | `/api/character*`, `/api/characters/{id}/sprites`, `/api/teacher_persona*`, `/api/persona/generate`, `/api/worlds*`, `/api/worlds/{id}/characters`, `/api/worlds/{id}/courses`, `/api/courses/{id}` |
...
| 类别 | 可直接参考内容 | 参考来源 |
|---|---|---|
| 视觉 Token | 颜色、字体、圆角、边框、阴影、blur 参数 | `UI_figma/src/styles/galgame.css` + 附录第 2 节 |
| 动效节奏 | keyframes 名称、时长、延迟、打字机速率 | `UI_figma/src/styles/galgame.css` + 附录第 3 节 |
| 页面分层 | Learning 四层结构、Home phase 结构、Archive/Settings 版式 | `UI_figma/src/app/pages/*.tsx` |
| 组件外观契约 | DialogBox/HudBar/CharacterSprite/SaveLoadPanel/KnowledgeGraphModal 的状态-样式映射 | 附录第 6 节 |
| 验收标准 | 像素与交互一致性检查项 | 附录第 9 节 |

### 7.2 需要对照仿写（不可直接包含 UI_figma 实现）

| 类别 | 处理方式 | 原因 |
|---|---|---|
| React 页面与组件代码 | 在 `frontend/src` 用 Vue SFC 重写 | 技术栈不同（React -> Vue） |
| React Router 路由语义 | 映射到现网路由（`/home`, `/learning/:courseId`, `/archive`） | UI_figma 路由是演示路径 |
| `mockData.ts` 与静态脚本 | 用当前仓库后端 API 替代，mock 仅可作开发占位 | 生产链路必须走真实业务 |
| Recharts/固定图谱逻辑 | 按现网 ECharts + D3/后端图谱结构重写 | 现网组件与数据形态不同 |
| 设置页本地状态项 | 区分后端设置项与本地偏好项，分别落地 | 避免把展示态误当后端能力 |
| 存档/分叉交互 | 按 checkpoint/branch 真实语义实现 | 现网是可持久化业务，不是纯 UI 演示 |

### 7.3 判定规则（避免跑偏）

1. **涉及接口/鉴权/持久化**：以本文第 2~6 节为唯一约束，不能退回纯 mock。  
2. **涉及视觉和动效细节**：以附录（UI_figma）参数为优先基线。  
3. **冲突时处理顺序**：后端业务契约 > 现网路由语义 > UI_figma 演示实现。

---

## 附录 A：「知遇」Galgame 学习系统 — Vue 3 UI 迁移技术规范（Figma/UI_figma 对照）

> **版本**：v1.0  
> **基准代码分支**：React 实现（Figma Make 生成，当前运行版本）  
> **冻结日期**：2026-04-05  
> **迁移目标**：Vue 3 + Vue Router + Pinia + Tailwind CSS v4  
> **范围**：样式、交互、动效迁移；并按当前仓库后端契约完成真实联调（见第 10 节）

> 本附录用于视觉与交互对照，**不覆盖**本文第 2~6 节的后端业务契约。
...
> 注：上表路由为 UI_figma 原始路由，落地到当前仓库时需映射为现网 Vue 路由。
...
### 7.1 核心类型定义（mockData → 当前仓库接口适配层）

| Mock 字段（TypeScript） | 适配层目标字段（前端内部） | 类型 | 转换规则 | 缺失时兜底 |
|---|---|---|---|---|
...
| 后端返回空 `choices` 数组 | 强制 mode='speaking' | 不显示选项区域 |
...
| 当前库 | Vue 等价 | 安装 |
|---|---|---|
| `motion/react` | **@vueuse/motion v3.0.3** | `npm install @vueuse/motion` |
| `recharts` | **vue-chartjs** | `npm install vue-chartjs chart.js` |

> **✅ 最终技术决策**（2026-04-05 技术研究）：
> 
> - **motion/react → @vueuse/motion**：motion/react 专为 React 设计，Vue 3 需使用 @vueuse/motion
> - **@vueuse/motion 优势**：
>   - v-motion 指令与 motion.div 语义一致
>   - 支持 variants、transition、delay 等完整功能
>   - 与 Vue 3 Composition API 完美集成
>   - <25kb bundle size
> - **recharts → vue-chartjs**：Chart.js 的 Vue 3 封装，支持折线图、饼图

> **v-motion 指令用法速查**：
> ```vue
> <!-- 等价于 React 的 motion.div -->
> <div v-motion :initial="{opacity:0,y:100}" :enter="{opacity:1,y:0}" />
> 
> <!-- 带延迟的列表动画 -->
> <div v-motion v-for="(item,i) in items" :key="item.id"
>   :initial="{opacity:0,y:30}" :enter="{opacity:1,y:0}"
>   :transition="{delay:i*0.12, duration:0.5}" />
> 
> <!-- AnimatePresence 等价：Vue <Transition mode="out-in"> -->
...
| ✅ 路由迁移 | UI_figma 路由语义映射到现网 Vue 路由（`/home`, `/learning/:courseId`, `/archive`, `/character`, `/settings`） |
| ✅ 状态管理迁移 | useState/useEffect → Pinia store 或 `<script setup>` + `ref/reactive` |
| ✅ 后端 API 联调 | 全量对接本文第 2 节接口，替换 UI_figma mockData 主链路 |
| ✅ 鉴权与会话联动 | 登录/鉴权头注入/路由守卫按现网实现保留 |
| ✅ 数据持久化链路 | checkpoint COMMIT/BRANCH、历史消息、图谱查询走真实后端 |
| ✅ Legacy 兼容 | Archive 对 `/api/save*` 的兼容读取/删除在迁移期保留 |

### 10.2 本次迁移不包含（可后续立项）

| 类别 | 说明 | 后续阶段建议 |
|---|---|---|
| ❌ Live2D / 3D 角色系统 | 继续使用现有静态立绘（sprites） | Phase X |
| ❌ 场景 AI 实时生成 | 保持现有场景资源/URL 方案 | Phase X |
| ❌ 全端像素级适配 | 本次保证可用与主断点一致性，不做所有机型逐像素校准 | Phase X |
| ❌ 全量视觉资源本地化 | 允许先使用现有可用外链与本地混合方案 | Phase X |

### 10.3 判定规则（避免范围漂移）

- **收到“能不能先用 mock 顶上”** → 仅允许开发占位，合入前必须切回真实 API 链路。  
- **收到“这个样式感觉不对”** → 对照本文附录第 2 节数值，数值内可修；超出需 Owner 确认。  
- **收到“顺便改业务字段/接口”** → 先对照本文第 2~6 节，不得破坏既有契约。  
